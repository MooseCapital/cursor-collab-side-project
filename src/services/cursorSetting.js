...
function setCursor({ cursorColor, cursorRGBA, id }) {
...
function setOthersCursor({ cursorColor, cursorRGBA, id, region, countryCode, flag }) {
    ...
    if (othersCursorCheckbox.checked) {
        ...
        data="${import.meta.env.BASE_URL}/images/bibata-${cursorColor}.svg"
        width="auto" height="max-content"></object> `,
        );
    }
}
...
/* function setLocationCursor(cursorColor, cursorRGBA) {
    const text = userData.region && userData.region.length < 3 ? userData.region : userData.countryCode;
...
//standard setCursor backup without location
/* function setCursor(color, colorRGBA) {
