...
function setOthersCursor({ cursorColor, cursorRGBA, id, webrtcId, region, countryCode, flag }) {
    ...
    if (othersCursorCheckbox.checked && ( !document.querySelector(`.other-cursor[data-webrtcid="${webrtcId}"]`) ||
        !document.querySelector(`.single-svg-cursor[data-webrtcid="${webrtcId}"]`)) ) {
            ...
            const containerString = `
                <div class="cursorContainer" data-webrtcid="${webrtcId}">
    ...
    } else if ( !document.querySelector(`.other-cursor[data-webrtcid="${webrtcId}"]`) || !document.querySelector(`.single-svg-cursor[data-webrtcid="${webrtcId}"]`) ) {
        
        const containerString = `
              <img class="single-svg-cursor" data-webrtcid="${webrtcId}" src="${cursorPath}" alt="Other users cursor icon" />
            `
